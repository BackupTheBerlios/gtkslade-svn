...
void end_linedraw()
{
	...
	if (line_draw && ldraw_points.n_points > 2) // Line drawing finished without error/cancellation
	{
...
void line_drawpoint()
{
	if (sel_box.x1() == -1)
	{
		if (!ldraw_points.add(point2_t(snap_to_grid(m_x(mouse.x)), snap_to_grid(-m_y(mouse.y))), false))
			end_linedraw();
	}
	else
	{
...
{
	ldraw_points.clear();
	ldraw_points.add(point2_t(snap_to_grid(m_x(sel_box.x1())),
							  snap_to_grid(-m_y(sel_box.y1()))), false);
	ldraw_points.add(point2_t(snap_to_grid(m_x(sel_box.x2())),
							  snap_to_grid(-m_y(sel_box.y1()))), false);
	ldraw_points.add(point2_t(snap_to_grid(m_x(sel_box.x2())),
							  snap_to_grid(-m_y(sel_box.y2()))), false);
	ldraw_points.add(point2_t(snap_to_grid(m_x(sel_box.x1())),
							  snap_to_grid(-m_y(sel_box.y2()))), false);
	ldraw_points.add(point2_t(snap_to_grid(m_x(sel_box.x1())),
							  snap_to_grid(-m_y(sel_box.y1()))), false);
